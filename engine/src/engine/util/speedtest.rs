...
use crate::{
    ...
    engine::{
        ...
        search::{
            NullReporter, PersistentState, TimeControl, time_control::StopControl,
            types::SearchResults,
        },
        uci::{ThreadCommand, Threads},
    },
};
...
pub fn speedtest(threads: Option<u64>, hash: Option<u64>, duration: Option<u64>) {
    ...
    let duration = duration.unwrap_or(120);
    let time_per_position = Duration::from_secs(duration) / POSITIONS.len() as u32;

    let state = Arc::new(PersistentState::new(hash as usize));

    let mut threads = Threads::new();
    threads.scale(nthreads as usize);

    let opts = |game: Game| {
        let state = state.clone();
        ThreadCommand::Search {
            game,
            time_control: TimeControl::ExactTime {
                time: time_per_position,
    ...
    for (index, &position) in POSITIONS.iter().enumerate() {
        ...
        nodes += state.node_counter.load(Ordering::Relaxed);
    }
    eprintln!();
